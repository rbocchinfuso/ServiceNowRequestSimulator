...
def temp_check():
    ...
    temp_fahrenheit = float(decimal.Decimal(random.randrange(int(low_temp)*100, int(high_temp)*100))/100)
...
def main():
    ...
    while (count < 10):
        ...
        t = Texttable()
        t.set_cols_width([20,15,20,15,15,10])
        t.set_cols_align(['c','c','c','c','c','c'])
        t.add_rows([['requester', 'location', 'temp_fahrenheit', 'temp_result', 'ppe_result', 'access'], [requester, location, temp[0], temp[1], ppe, access[0]]])
        print(t.draw())
...
if __name__ == '__main__':
    ...
    c = pysnow.Client(instance=(config['now_api']['instance']), user=(config['now_api']['user']), password=(config['now_api']['password']))
    # set cycle time
